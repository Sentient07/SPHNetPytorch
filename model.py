...
class PointNetWithSPH(nn.Module):
    ...
    def forward(self, inp_pc):
        ...
        x = self.kdTreePool_2(F.relu(self.bn2(self.conv2(inp_downsampled, x, return_filters=False).transpose(1,2)))).transpose(1,2)
        # x = self.kdTreePooling(F.relu(self.conv2(inp_downsampled, x, return_filters=False).transpose(1,2))).transpose(1,2)
        ...
        # x = F.relu(self.conv3(inp_downsampled, x, return_filters=False).transpose(1,2)).transpose(1,2)
        x, _ = torch.max(x, 1)
#         x = x.view(-1, self.nlatent)

        x = self.drop1(F.relu(self.bn4(self.lin1(x))))
        # x = self.drop1(F.relu(self.lin1(x).unsqueeze(-1)))


        x = self.drop2(F.relu(self.bn5(self.lin2(x))))
        # x = self.drop2(F.relu(self.lin2(x.squeeze(2)).unsqueeze(-1)))
